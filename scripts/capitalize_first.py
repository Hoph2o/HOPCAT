...
CAT_LABEL = "Capitalize first lyric in phrases"
CAT_CATEGORY = "Vocals"
CAT_ORDER = 1
...
def execute(sel):
    ...
    C3toolbox.startup()
    C3toolbox.capitalize_first(instrument, sel) #instrument, level, option, selected
    form.destroy()
...
def launch():
    ...
    form.wm_title('Capitalize first letter in phrase')
    C3toolbox.startup()
    ...
    allBtn = tkinter.Button(helpLf, text="Fix all phrases", command= lambda: execute(0)) 
    allBtn.grid(row=0, column=2, rowspan=1, sticky="WE", padx=5, pady=2)

    selBtn = tkinter.Button(helpLf, text="Fix selected phrases only", command= lambda: execute(1)) 
...
if __name__ == '__main__':
    launch()
    #C3toolbox.startup()
    #C3toolbox.capitalize_first('PART VOCALS', 0) #Do not set SELECTED to 1, it won't work
