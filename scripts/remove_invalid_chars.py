...
CAT_LABEL = "Remove illegal punctuation"
CAT_CATEGORY = "Vocals"
CAT_ORDER = 1
...
def execute(sel):
    ...
    C3toolbox.startup()
    C3toolbox.remove_invalid_chars(instrument, sel) #instrument, level, option, selected
    form.destroy()
...
def launch():
    ...
    form.wm_title('Remove illegal punctuation')
    C3toolbox.startup()
    ...
    allBtn = tkinter.Button(helpLf, text="Fix all lyrics", command= lambda: execute(0)) 
...
if __name__ == '__main__':
    launch()
    #C3toolbox.startup()
    #C3toolbox.remove_invalid_chars('PART VOCALS', 0) #Do not set SELECTED to 1, it won't work
