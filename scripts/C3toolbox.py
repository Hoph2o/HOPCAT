...
def center_on_screen(win):
    """Center a Tk window on the primary screen."""
    try:
        ...
        RPR_ShowConsoleMsg("center_on_screen error: {}\n".format(e))
...
def remap_notes(array_chords, array_translation): #Remap all chords in a song to allowed chords
    array_chords_sorted = sorted(list(array_chords.items()), key=operator.itemgetter(1), reverse=True)
    array_valid_chords = []
...
def get_trackid():  # Returns the media-item index for the currently selected track
    global tracks_array
    prep_tracks()  # keep the map current every time

    sel = RPR_CountSelectedTracks(0)
    if sel != 1:
        return ""  # more than one or none selected

    tr = RPR_GetSelectedTrack(0, 0)
    source = RPR_GetSetMediaTrackInfo_String(tr, "P_NAME", "", 0)[3]

    # Normalize odd names to the canonical keys used in tracks_array
    alias_map = {
        "LEAD GUITAR": "PART GUITAR",
        "GUITAR": "PART GUITAR",
        "RHYTHM GUITAR": "PART RHYTHM",
        "BASS": "PART BASS",
        "DRUMS": "PART DRUMS",
        "VOCALS": "PART VOCALS",
        "KEYS": "PART KEYS",
    ...
    return tracks_array.get(source, 999)
...
def process_instrument(instrument):  # Creates an array of all notes/events
    ...
    if not mi:
        RPR_MB("Invalid media item index ({}). Is the right track selected?".format(instrument),
               "CAT invalid selection", 0)
        # ticks defaults to correct_tqn (480) so callers don't crash
        return [int(correct_tqn), [], 0, 0]
    ...
    ticks = int(correct_tqn)          # default to 480
    end_firstpart = 0
    start_part = 0
    notes_array = []
    end_event = 0
    ...
    for j in range(0, len(vars_array)):
        ...
        if line.startswith('E ') or line.startswith('e '):
            ...
            if len(note) >= 2:
                ...
                try:
                    if base64.b64decode(encText.encode())[2:] == b'[end]':
                        end_event = noteloc
                except Exception:
                    pass

        elif "HASDATA" in line:
            # Example: "HASDATA 1 480" -> third token is ticks/quarter
            parts = line.split()
            if len(parts) >= 3:
                try:
                    ticks = int(parts[2])
                except Exception:
                    ticks = int(correct_tqn)
            if ticks != 480:
                RPR_MB("One of the MIDI items isn't set to 480 ticks per quarter. "
                       "This can break Magma. Using it anyway.", "Ticks per quarter", 0)

        elif "<SOURCE MIDI" in line:
            end_firstpart = j + 2  # last element before notes/events chunk

        elif "IGNTEMPO" in line:
            start_part = j - 1     # first element after notes/events chunk

    # If no notes/events, keep end_of_track safe and return
    if not notes_array:
        end_of_track = "e 0 90 00 00"  # safe dummy
        return [ticks, notes_array, end_firstpart, start_part]
...
def create_notes_array(notes): #instrument is the instrument shortname, NOT the instrument track number
    ...
    for x in range(0, len(array_rawevents)):
        ...
        encText = note_bit[3]
        event_header = binascii.hexlify(binascii.a2b_base64(encText)).decode()[:4]
        lyric = base64.b64decode(str(encText))[2:]
        array_events.append([note_bit[0], int(note_bit[1]), note_bit[2], lyric.decode(), note_bit[4], event_header])
...
def rebuild_array(array_notesevents):
    ...
    for x in range(0, len(array_notesevents[1])):
        hex_lyric = array_notesevents[1][x][5] + binascii.hexlify(array_notesevents[1][x][3].encode("utf-8")).decode()
        encoded_text = base64.encodebytes(binascii.a2b_hex(hex_lyric)).decode()
...
def prep_tracks():
    ...
    for i in range(0, num_mi):
        ...
        elif "PART RHYTHM" or "RHYTHM" == trackname:
...
def create_beattrack(halve, sel): #Call with halve to create an halved beat track
    ...
    if "EVENTS" in tracks_array:
        process_instrument(tracks_array["EVENTS"])  # sets end_event
...
def remove_invalid_chars(instrument, selected):
    ...
    for x in range(0, len(array_validevents)):
        ...
        if event[5] == 'ff05':
            table = str.maketrans(dict.fromkeys(invalid_chars))
            event[3] = event[3].translate(table)
...
def create_phrase_markers(instrument, grid, mute):
    ...
    for x in range(0, len(array_validevents)):
        ...
        if phrase_char in event[3] and start == 0: #We need to start a phrase
            ...
            start = event[1]
            table = str.maketrans(dict.fromkeys(phrase_char))
            event[3] = event[3].translate(table)
...
def startup():
    ...
    array_instrument_data = process_instrument(tracks_array["EVENTS"])
