...
def execute(selected):
    ...
    C3toolbox.startup()
    C3toolbox.flip_discobeat(instrument, C3toolbox.array_levels[level][0], selected, 0)
...
def launch():
    ...
    C3toolbox.startup()
    form = tkinter.Tk()
    form.wm_title('Flip disco beat sections')
    ...
    allBtn = tkinter.Button(helpLf, text="Flip all", command= lambda: execute(0)) 
    allBtn.grid(row=0, column=3, rowspan=1, sticky="WE", padx=5, pady=2)

    selBtn = tkinter.Button(helpLf, text="Flip selected", command= lambda: execute(1)) 
