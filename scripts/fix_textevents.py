...
def execute():
    ...
    grid_array = { 'Fix all issues' : 0, 'Fix only lyrics marked as text' : 1, 'Fix only text events marked as lyrics' : 2 }
    grid = grid_array[grid]
    C3toolbox.startup()
    C3toolbox.fix_textevents(instrument, grid) #instrument, level, option, selected
    form.destroy()
...
def launch():
    ...
    form.wm_title('Fix text markers')
    C3toolbox.startup()
    ...
    gridOpt = tkinter.OptionMenu(helpLf, grid_var, *OPTIONS)
    gridOpt.grid(row=0, column=2, columnspan=1, sticky="WE", pady=3)

    allBtn = tkinter.Button(helpLf, text="Fix text markers", command= lambda: execute()) 
...
if __name__ == '__main__':
    launch()
    #C3toolbox.startup()
    #C3toolbox.fix_textevents('PART VOCALS', 0) #Do not set SELECTED to 1, it won't work
