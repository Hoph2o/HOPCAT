...
CAT_LABEL = "Change notes to non-pitched"
CAT_CATEGORY = "Vocals"
CAT_ORDER = 1
...
def execute(sel):
    ...
    character = str(expression_var.get())
    if character == 'Loose non-pitched (^)':
        character = '^'
    else:
        character = '#'
    ...
    C3toolbox.startup()
    C3toolbox.unpitch(instrument, character, sel) #instrument, level, option, selected
    form.destroy()
...
if __name__ == '__main__':
    launch()
    #C3toolbox.startup()
    #C3toolbox.unpitch('PART VOCALS', '#', 0) #Do not set SELECTED to 1, it won't work
