...
def execute(sel):
    ...
    C3toolbox.startup()
    C3toolbox.add_slides(instrument, sel) #instrument, level, option, selected
    form.destroy()

def onKeyPress(event):
    C3toolbox.PM('>>>' + (event.char) + '<<<')

def launch():
    global instrument_var
    global form
    
    # RPR_OnPlayButtonEx(0);
    form = tkinter.Tk()
    getFld = tkinter.IntVar()
    form.wm_title('Add slides')
    C3toolbox.startup()
    ...
    allBtn = tkinter.Button(helpLf, text="Add all missing slides", command= lambda: execute(0)) 
    allBtn.grid(row=0, column=2, rowspan=1, sticky="WE", padx=5, pady=2)

    selBtn = tkinter.Button(helpLf, text="Add missing slides to selected notes", command= lambda: execute(1)) 
...
if __name__ == '__main__':
    launch()
    #C3toolbox.startup()
    #C3toolbox.add_slides('PART VOCALS', 1)
